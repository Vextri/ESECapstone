...
Right ascending: Component Verification, Subsystem Verification, Integration Verification, System Validation, User Acceptance.
Horizontal links joining each design level to its corresponding test level, annotated with which of your five test plans applies at each level.
...
L3, Integration	Interaction across subsystem boundaries on the assembled unit	System Integration and Validation Test Plan, Part A	Both	Section 9.1
L4, Validation	Assembled system against the requirements of Section 3	System Integration and Validation Test Plan, Part B; Functional Prototype Test Plan	Both	Section 9.2
...
Wk 13	Final assembly, functional prototype validation, long-duration reliability trial, final demonstration	[PLACEHOLDER: complete this row once Week 13 work is done]
...
[PLACEHOLDER: CONFIRM DROP SITE 3 RESOLVED] Assessment 2 recorded drop site 3 as unusable. Confirm in Section 9.1 and 9.5 whether the PCB build and rerouted harness fully resolved this, and provide a photograph of the cleared drop path. If any slot remains impaired, say so, an evaluator who watches a three-slot demonstration in which only two slots dispense will ask.
...
Isolation. The fault was invisible to conventional debugging: the assembled enclosure leaves no room to attach a serial cable to the ESP32-S3, so the normal diagnostic channel was physically unavailable. This constraint directly motivated the construction of the in-device remote log viewer described in Section 7.5.6. With network-accessible logging in place, the fault was captured live: two separate commands were observed merging into a single corrupted message.
...
[PLACEHOLDER: VERIFICATION STATUS] The record marks this "implemented a fix and verified." State how it was verified, how many dispense cycles were run with background synchronisation active, and over what period, without recurrence. An intermittent defect requires evidence of absence over a meaningful sample, not a single successful run.
...
Isolation: The remote log viewer captured ENFILE errors, the operating system reporting no file descriptors available, appearing within seconds of the second device joining.
...
A validation test for logical per-station state independence has been added to Part A of the System Integration and Validation Test Plan (INT-12), alongside the existing U-01 sensor independence test.
